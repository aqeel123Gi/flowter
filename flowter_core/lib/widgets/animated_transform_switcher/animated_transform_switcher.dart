...
class AnimatedTransformingSwitcher<T> extends StatefulWidget {
  ...
  @override
  State<AnimatedTransformingSwitcher<T>> createState() =>
      _AnimatedTransformingSwitcherState<T>();
}

class _AnimatedTransformingSwitcherState<T>
    extends State<AnimatedTransformingSwitcher<T>> {
  bool init = false;
  ...
  void _addIfNotTheSame() {
    if (_currentKey != widget.switcher) {
      ...
      _children.add(IgnorePointer(
          ignoring: true,
          child: Animate(
              key: _keys.last,
              controller: _controllers.last,
              startFrom: TransformData(
                  opacity: 0,
                  x: widget.startX,
                  y: widget.startY,
  ...
  Widget build(BuildContext context) {
    ...
    return Stack(children: [
      ...
      Animate(
          ...
          controller: _controllers.last,
          startFrom:
              TransformData(opacity: 0, x: widget.startX, y: widget.startY),
