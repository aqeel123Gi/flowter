...
class AnimateController {
  ...
  static startOneByOneByID<T>(
      String staticSubID, List<T> list, int milliseconds) {
...
class _AnimateState extends State<Animate> {
  ...
  isShown() {
    return _currentAnimatedTransformData.x == 0 &&
        _currentAnimatedTransformData.y == 0 &&
        _currentAnimatedTransformData.opacity == 1;
  }
  ...
  show() {
    if (mounted) {
      Future.delayed(widget.showAfter, () {
        if (mounted) {
          setState(() {
            _currentAnimatedTransformData = widget.startFrom!
                .copyWith(duration: Duration.zero, ignorance: false);
          });
  ...
  void initState() {
    ...
    if (widget.startFrom != null) {
      _currentAnimatedTransformData =
          widget.startFrom!.copyWith(ignorance: true);
  ...
  Widget build(BuildContext context) {
    return AnimatedTransform(
        data: _currentAnimatedTransformData, child: widget.child);
  }
}
