      ...
      children: [
        AnimatedContainer(
          ...
          height: widget.currentIndex == index
              ? widget.currentTabPointSize
              : widget.tabPointsSize,
          ...
          child: FilledButtonAsset(
            onPressed: widget.points[index].enabled
                ? () => widget.onTabPointPressed(index)
                : null,
          ),
        ),
        !widget.shownText
            ? const SizedBox()
            : SizedBox(width: widget.spaceBetweenTextsAndPoints),
        !widget.shownText
            ? const SizedBox()
            : SizedBox(
                ...
                child: AnimatedDefaultTextStyle(
                    style: widget.currentIndex == index
                        ? widget.currentTabPointTextStyle
                        : widget.points[index].enabled
                            ? widget.otherEnabledTabPointTextStyle
                            : widget.otherDisabledTabPointTextStyle,
                    curve: widget.curve,
                    duration: widget.duration,
  ...
  Widget outline(int index) {
    return AnimatedTransformingSwitcher(
        ...
        builder: (context, switcherKey) => Center(
            child: Text(tryGet(() => widget.points[index].title, "")!,
                style: widget.currentTabPointTextStyle)));
  }
        ...
        ? Column(
            ...
            children: [
              SizedBox(
                ...
                child: Row(
                    mainAxisAlignment: MainAxisAlignment.center,
                    children: List.generate(widget.points.length,
                            (index) => tabPointWidgetHorizontal(index))
                        .insertBetweenElements(widget.axis == Axis.horizontal
                            ? SizedBox(width: widget.spaceBetweenPoints)
                            : SizedBox(width: widget.spaceBetweenPoints))),
              ),
              !widget.shownText
                  ? const SizedBox()
                  : SizedBox(height: widget.spaceBetweenTextsAndPoints),
              !widget.shownText
                  ? const SizedBox()
                  : Center(child: outline(widget.currentIndex))
            ],
          )
        : Column(
            mainAxisAlignment: MainAxisAlignment.center,
            children: List.generate(widget.points.length,
                    (index) => tabPointWidgetVerticalWithOutline(index))
                .insertBetweenElements(widget.axis == Axis.horizontal
                    ? SizedBox(width: widget.spaceBetweenPoints)
                    : SizedBox(height: widget.spaceBetweenPoints)));
  }
}
