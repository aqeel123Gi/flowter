              ...
              Stack(
                  children:[
                    ...
                    AnimatedPositioned(
                      curve: _curve,
                      duration: Duration(milliseconds: _durationSeconds),
                      bottom: _bottom-_draggedBottom+MediaQuery.of(context).viewInsets.bottom,
                      left: 0,
                      right: 0,
                      child: GestureDetector(
                          onVerticalDragStart: widget.cancelByDragging?(p){
                            _touchY = p.globalPosition.dy;
                          }:null,
                          onVerticalDragUpdate: widget.cancelByDragging?(p){
                            setState((){
                              _draggedBottom = p.globalPosition.dy-_touchY;
                              _draggedBottom =_draggedBottom<0?0:_draggedBottom;
                            });

                          }:null,
                          onVerticalDragEnd: widget.cancelByDragging?(p){
                          ...
                          child:Blur(
                              ...
                              decoration:widget.decoration, child: ChildSizeDetector(
                            child:widget.child,
                            onChange: (Size size) {
                              if(!initialized){
                                initialized=true;
