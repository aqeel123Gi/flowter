...
void viewWidget(GlobalKey widgetKey, Widget widget) {
  TemplateController.push(TemplateData(
      id: "WIDGET_VIEWER",
      body: WidgetViewer(initEdgesCoordinates: getEdgesFromKey(widgetKey)!,widget: widget),
...
class _WidgetViewerState extends State<WidgetViewer> {
  ...
  bool _hide(){
    setState(() {
      _offset = Offset(widget.initEdgesCoordinates.left,widget.initEdgesCoordinates.top);
      _rotation = 0;
      _scale = 1;
      _show = false;
    });
  ...
  void initState() {
    ...
    addPostFrameCallback(() {
      setState((){
        ...
        _show = true;
        _scale = scale();
        _offset = Offset((MediaQuery.of(context).size.width-_size.width)/2,(MediaQuery.of(context).size.height-_size.height)/2);
      });
    });
