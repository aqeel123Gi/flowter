...
class _CalenderState extends State<Calender> {
    ...
    return WidgetControllerBuilder<CalenderController>(
        ...
        builder: (context, c) {
          return UpdatingStateSwitcher(
            switchers: "${_controller.shownYear}-${_controller.shownMonth}-${widget.selectedDates}",
            child: Column(
                  children: [
                    _header,
                    const Space(24),
                    ...
                    AnimatedTransformingSwitcher(
                        ...
                        builder:(context, switcherKey)=> _days(_controller.shownYear, _controller.shownMonth))
                  ],
  ...
  Widget get _header => Row(children: [
    AnimatedTransformingSwitcher(
      duration: const Duration(milliseconds: 500),
      switcher: "${_controller.shownYear}-${_controller.shownMonth}",
      builder: (context, switcherKey)=> Text(parseDateTimeToString(DateTime(_controller.shownYear,_controller.shownMonth), "month ، yyyy"),
  ...
  Widget get _weekDays => Row(children:  [
    for (var e in [7,1,2,3,4,5,6])
      Expanded(
        child: Text(widget.weekDayText(e), style: widget.weekDaysTextStyle, textAlign: TextAlign.center),
      )
  ]);
  ...
  Widget _days(int year, int month) => Builder(builder: (context){
    ...
    List<List<Widget>> children = List.generate(5, (_)=>List.generate(7, (_)=>const Expanded(
    ...
    Widget addDay(int day) {
      ...
      bool available = widget.availableDateCondition==null || widget.availableDateCondition!(DateTime(year, month, day));
      bool selected = widget.selectedDates.any((date)=>isSameDay(date, DateTime(year, month, day)));
            ...
            child: Button(
                ...
                contentBuilder: (context, focused) =>
                    CenterRegardlessLayout(
                      verticalShift: 4,
                      child: Text(
                          toStringWithZerosDigits(day, 2), style: selected?widget.daysOnSelectedTextStyle:available?widget.daysTextStyle:widget.daysOnNotAvailableTextStyle),
                    ),
                decoration: selected?widget.daysOnSelectedBoxDecoration:widget.daysBoxDecoration,
            onPressed: available?()=>_controller.onDaySelected(DateTime(year, month, day)):null,
    ...
    for(var r=0; r<5; r++){
      for(var c=0; c<7; c++){
        if(startFilling && DateTime(year, month).add(Duration(days: d)).month == month){
          d++;
