...
class _TabsState extends State<Tabs> {
  ...
  Widget build(BuildContext context) => Stack(
      children: [
        AnimatedPositioned(
          ...
          bottom: widget.shown && init ? 0 : -(MediaQuery.of(context).padding.bottom+widget.tabSize+(widget.viewVerticalPadding*2)),
          child: Container(
            padding: EdgeInsets.symmetric(horizontal: widget.viewHorizontalPadding, vertical: widget.viewVerticalPadding),
            decoration: widget.viewDecoration,
            child: Padding(
              padding: EdgeInsets.only(bottom:MediaQuery.of(context).padding.bottom),
              child: Row(
                  mainAxisAlignment: MainAxisAlignment.spaceBetween,
                  children: widget.tabs.build((i,e) => _tabButton(i==widget.activeTab,i,e))),
            ),
          ))
      ]);
  ...
  _tabButton(bool active, int index, ButtonData tab) {
    ...
    return Button(
      ...
      decoration: active
          ? widget.tabOnActiveDecoration
          : widget.tabDecoration,
      decorationOnFocused: active ? widget
          .tabOnActiveDecoration : widget.tabOnFocusedDecoration,
        ...
        return Row(
                  children: [
                    SizedBox(
                        ...
                        child: tab.icon != null? tab.icon!.withParams(size: widget.iconSize, color: active?widget.iconColorOnActive:widget.iconColor) : Icon(tab.iconData, size: widget.iconSize, color: active?widget.iconColorOnActive:widget.iconColor)),
                    AnimatedTransform(
                        ...
                        child:Row(
                          children: [
                            Text(
                                ...
                                tab.titleBuilder!=null?tab.titleBuilder!():
  ...
  TransformData hiddenTransform(int tabIndex)=>TransformData(opacity: 0, y:widget.verticalTextShifting, x: widget.iconSize, width: 0, duration: widget.tabAnimation.duration, curve: widget.tabAnimation.curve);
  TransformData appearTransform(int tabIndex)=>TransformData(opacity: 1, y:widget.verticalTextShifting, x: 0, width: tabsWidth[tabIndex], duration: widget.tabAnimation.duration, curve: widget.tabAnimation.curve);
