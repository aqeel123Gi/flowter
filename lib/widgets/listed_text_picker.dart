...
class _ListedTextsPickerState extends State<ListedTextsPicker> {
  ...
  void _onPointerMove(PointerMoveEvent e){
    if(_startOffset!=null){
      _controller.jumpTo(_startOffset! + PublicListener.pointerDownEvent!.position.dy - e.position.dy);
  ...
  void _onPointerUp(PointerUpEvent e){
    ...
    if(_startOffset!=null){
      ...
      if((e.position.dy-_moves.getFromLast(2)).abs()<=2){
        ...
        _controller.animateTo(_controller.offset+((e.position.dy-_moves.getFromLast(2))*-12), duration: const Duration(milliseconds: 1000), curve: Curves.easeOutCirc);
  ...
  void _animate([int? index]){
    _controller.animateTo(
        (index??_getCurrentIndex())*widget.size.height,
  ...
  int _getCurrentIndex(){
    ...
    return i<0?0:i>=widget.choices.length?widget.choices.length-1:i;
  }
  ...
  void initState() {
    ...
    WidgetsBinding.instance.addPostFrameCallback((_) {
      _controller.jumpTo((widget.initChoice??_getCurrentIndex())*widget.size.height);
    });
    super.initState();
  }
                    ...
                    child:IgnorePointer(
                        child:ListView(
                          ...
                          children: widget.choices.map((e) => SizedBox(
                              height: widget.size.height,
                              child: Center(child:Text(e,style: widget.textStyle,textAlign: TextAlign.center))
                          )).toList()
                    )))
            ))
    ));
