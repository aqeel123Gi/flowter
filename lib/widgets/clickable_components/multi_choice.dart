          ...
          children: [
            Builder(builder: (context) {
              ...
              return Row(
                  children: <Widget>[
                for (var i in list)
                  Text(fromChoices[i]!, style: widget.choiceStyle),
              ].insertBetweenElements(
                      SizedBox(width: widget.choiceStyle.fontSize! * 2))
                    ..add(SizedBox(width: widget.height - 9)));
            })
          ]);

  Widget _dp(Widget child) => Padding(
      padding: EdgeInsetsDirectional.only(end: widget.height - 9),
      child: child);
                            ...
                            (widget.enabled &&
                                (widget.choices == null &&
                                    (widget.futureChoices == null ||
                                        widget.futureChoices!.pending ||
                                        widget.futureChoices!.data == null))))
                        ? widget.numberDecorationOnDisabled
                        : widget.numberDecoration,
                    child: Center(
                        child: Text(widget.chosen.length.toString(),
