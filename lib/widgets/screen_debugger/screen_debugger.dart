...
class DebuggerConsole extends StatefulWidget{
  ...
  static void Function(String key, String text, {Color? color}) setPinnedLine = (String key, String text, {Color? color}){ DebuggerConsoleData.pinnedLines[key] = MapEntry(text,color??Colors.green); };
  ...
  static void Function(String text, {Color? color}) addLine = (String text, {Color? color}){};
  static void Function(EdgesCoordinates edges, {Color? color}) addRectangle = (EdgesCoordinates edges, {Color? color}){};
  static void Function(Uint8List imageData) showImage = (Uint8List imageData){};
...
class _DebuggerConsoleState extends State<DebuggerConsole> {
  ...
  void _addPinnedLine(String key, String text, {Color? color}){
    ...
    setState(() {
      DebuggerConsoleData.pinnedLines[key] = MapEntry(text,color??Colors.green);
    });
  }
  ...
  void _addRectangle(EdgesCoordinates edges, {Color? color}){
    setState(() {
      DebuggerConsoleData.rectangles.add(MapEntry(edges,color??Colors.green));
    });
  }
                  ...
                  child: Stack(children:[
                    Stack(
                        children:[Positioned(
                            ...
                            right: 20,
                            width: 100,
                            child:DebuggerConsoleData.imageData!=null?Image.memory(DebuggerConsoleData.imageData!):const SizedBox())]
                    ),
                    Stack(
                        children:DebuggerConsoleData.rectangles.map((e) => Positioned(
                            top: e.key.top,
                            height: e.key.bottom-e.key.top,
                            left: e.key.left,
                            width: e.key.right-e.key.left,
                            child:Container(
                              decoration: BoxDecoration(border: Border.all(color:e.value,width:1)),
                            ))).toList()
                    ),
                    Align(alignment: Alignment.bottomLeft,child:Padding(padding:EdgeInsets.only(bottom: MediaQuery.of(context).padding.bottom*0.2),
                        child:ListView(
                            ...
                            [
                              ...
                              ...DebuggerConsoleData.texts.build((_,e)=> Align(alignment: Alignment.centerLeft,child:Container(
                                padding: const EdgeInsets.only(left: 3,right: 3,top: 3),
                                color: Colors.black.withValues(alpha:1),
                                child:Text(e.key,style: TextStyle(color: e.value,fontSize: 10, fontWeight: FontWeight.bold))))),


                              ...DebuggerConsoleData.pinnedLines.build((_,e)=>Align(alignment: Alignment.centerLeft,child:Container(
                                padding: const EdgeInsets.only(left: 3,right: 3,top: 3),
                                color: Colors.black.withValues(alpha:1),
                                child:Text(e.key,style: TextStyle(color: e.value,fontSize: 10, fontWeight: FontWeight.bold))))),
                    ...
                    DebuggerConsoleData.position!=null?Positioned(
