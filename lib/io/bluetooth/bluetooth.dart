...
class IOBluetooth {
  ...
  static Map<XBluetoothDevice, List<void Function(List<int> notifiedData)>> notifiersListeners = {};


  static List<void Function(List<int> notifiedData)> globalNotifiersListeners = [];


  static final List<void Function(XBluetoothAdapterState state)> _bluetoothStateListeners = [];


  static void addBluetoothStateListener(void Function(XBluetoothAdapterState state) function) {
    _bluetoothStateListeners.add(function);
  }


  static void removeBluetoothStateListener(void Function(XBluetoothAdapterState state) function) {
  ...
  static Future<void> _setCurrentAuth() async {
    ...
    if(Platform.isIOS){
      ...
      _auth = await Permission.bluetoothScan.isGranted && await Permission.bluetoothConnect.isGranted && await Permission.location.isGranted;
    }
  ...
  static Future<void> initialize({bool globalListenFromAllNotifiers = false}) async {
    switch(Platform.operatingSystem){
      ...
      case "ios":
      case "macos":
        await _IOBluetoothAndroidIosMacos.initialize(globalListenFromAllNotifiers: globalListenFromAllNotifiers);
        _initialized = true;
        break;
      case "windows":
        await _IOBluetoothWindows.initialize(globalListenFromAllNotifiers: globalListenFromAllNotifiers);
        _initialized = true;
        break;
      default:
  ...
  static Future<void> requestPermissionsForScanningAndConnection() async {
    switch(Platform.operatingSystem){
      ...
      case "ios":
      case "macos":
        await _IOBluetoothAndroidIosMacos.requestPermissionsForScanningAndConnection();
        break;
      case "windows":
  ...
  static Future<BluetoothScanningRequirements> getBluetoothScanningRequirements()async{
  ...
  static Future<List<XBluetoothDevice>> scan({required int seconds, bool filterSavedDevices = false}) async {
    switch(Platform.operatingSystem){
      ...
      case "ios":
      case "macos":
        return await _IOBluetoothAndroidIosMacos.scan(seconds: seconds, filterSavedDevices: filterSavedDevices);
      case "windows":
        return await _IOBluetoothWindows.scan(seconds: seconds, filterSavedDevices: filterSavedDevices);
      default:
        throw Exception("Unsupported platform: ${Platform.operatingSystem}");
    }
  }
  ...
  static Future<bool> connect(XBluetoothDevice device) async {
    if(IOBluetooth._adapter == XBluetoothAdapterState.off){
      throw Exception("Bluetooth is off");
    }
    try {
      await device.connect();
      if(IOBluetooth._globalListenFromAllNotifiers){
  ...
  static Future<bool> disconnect(String deviceId) async {
    switch(Platform.operatingSystem){
      ...
      case "macos":
      try {
        await FlutterBluePlus.connectedDevices.firstWhere((e)=>e.remoteId.str == deviceId).disconnect();
        return true;
      } catch (e, s) {
        if (kDebugMode) {
          print(e);
  ...
  static Future<void> setAllNotifiersOfDeviceAsTrueForGlobalListeners(XBluetoothDevice device) async {

    List<XBluetoothService> services = await device.services;
    for (XBluetoothService service in services) {
      for (XBluetoothCharacteristic characteristic in await service.characteristics) {
  ...
  static List<SavedDevice> get savedDevicesForAutoConnectionList => _savedDevicesForAutoConnectionList.copy;
  ...
  static Future<void> renameSavedDevice(SavedDevice savedDevice, String newName)async{
  ...
  {void Function(SavedDevice savedDevice, XBluetoothDevice connectedDevice)? onConnected}) async {
    _stopAutoConnection = false;
    await _getDevices();
    loopExecution(function: () async => await _autoConnect(onConnected), stopOn: () => _stopAutoConnection,breakDuration: const Duration(seconds: 5));
  }
  ...
  static Future<void> _autoConnect([void Function(SavedDevice savedDevice, XBluetoothDevice connectedDevice)? onConnected]) async {
    //print("AutoConnect: Saved Devices: ${_savedDevicesForAutoConnectionList.length}");
    if (_savedDevicesForAutoConnectionList.isNotEmpty &&!_pausedAutoConnection && adapterState == XBluetoothAdapterState.on) {
      // Get not connected Devices :
      List<SavedDevice> notConnectedDevices = (await _savedDevicesForAutoConnectionList.whereAsync((savedDevice) async{
      ...
      for (SavedDevice savedDevice in notConnectedDevices) {
        ...
        if (devices.isNotEmpty) {
          XBluetoothDevice? device = devices.tryFirstWhere((element) =>
          element.id == savedDevice.id);
  ...
  static bool isSavedDeviceConnected(SavedDevice savedDevice)=> pairedIds.any((id) => id == savedDevice.id);
  ...
  static Future<bool> connectAndAddToAutoConnectionList(XBluetoothDevice device, String name)async{
    if(await connect(device)){
      _savedDevicesForAutoConnectionList.add(SavedDevice(id: device.id, name: name, type: DeviceType.uhf));
      await _saveDevices();
      return true;
    }else{
      return false;
  ...
  static Future<void> removeFromAutoConnectionList(String deviceID, BluetoothDevice device)async{
    _savedDevicesForAutoConnectionList.removeWhere((savedDevice) => savedDevice.id ==(deviceID));
    await _saveDevices();
  }
...
class BluetoothScanningRequirements{
  ...
  static Future<BluetoothScanningRequirements> _check() async {
    ...
    return BluetoothScanningRequirements._(
      ...
      hasLocationAccessPermission: (await Geolocator.checkPermission()).execute((permission)=> permission!= LocationPermission.denied && permission!=LocationPermission.deniedForever),
      isBluetoothAuthorized: IOBluetooth.authorizedScanningAndConnection,
      isBluetoothOn: IOBluetooth.adapterState == XBluetoothAdapterState.on,
      isLocationOn: await Geolocator.isLocationServiceEnabled(),
    );
  }

  bool get completed => hasLocationAccessPermission && isBluetoothAuthorized && isBluetoothOn && isLocationOn;
