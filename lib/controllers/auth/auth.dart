...
class AuthController{
  ...
  static Future<void> initialize(Map<Type, AbstractAppAuth Function(Map<String, dynamic>)> possibleAuthTypeWithFromMemoryAsMapFunctions, {String? fileName}) async {
    ...
    if(type != null){
      try{
        MapEntry<Type, AbstractAppAuth Function(Map<String, dynamic>)> entry = possibleAuthTypeWithFromMemoryAsMapFunctions.entryWhereFirstKey((possibleAuthType) => possibleAuthType.toString() == type);
        _user = entry.value(json.decode(body));
      }catch(e){
        throw Exception('Type $type is not found in ${possibleAuthTypeWithFromMemoryAsMapFunctions.keys.toList()}');
      }
    }
  }
  ...
  static Future<void> set({required String token, AbstractAppAuth? user}) async {
